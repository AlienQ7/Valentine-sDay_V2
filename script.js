...
function handleNoClick() {
    ...
    const messages = getMessages(); // Get the updated messages each time
    ...
    // Increase Yes button size on every "No" click
    yesButton.style.fontSize = `${currentYesSize * 1.5}px`;

    // When the last message is displayed, remove the No button and shrink the Yes button
    if (currentMessage === "Your trapped until you say yes!!") {
        noButton.remove();
        yesButton.style.fontSize = `${currentYesSize / 2}px`; // Make it smaller
    }
}
